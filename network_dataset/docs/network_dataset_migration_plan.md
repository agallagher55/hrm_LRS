...
`TRNLRS_street_network` has been built inside `SDEADM.TRNLRS_network` in both
**Dev and QA** (2026-07-14) -- prod is still on the earlier layout where these
FCs live inside `SDEADM.TRNLRS` alongside the LRS data.

The intended move path (verify feature dataset + spatial reference match,
copy each FC, verify feature counts, then delete the originals) is
implemented in `network_dataset/scripts/06_migrate_network_fd.py`. In practice, `06` was not
run before `03` in either Dev or QA, so `SDEADM.TRNLRS_network` was empty when
`03_create_network_dataset.py` ran and its own fallback copy logic populated
the three sources instead -- pulling the edge source fresh from prod's
`TRNLRS_TRN_STREET_VW` (fine) and the junction/turn sources fresh from the
legacy `SDEADM.TRN_streets_routes` (fine for the junction, but the *raw,
unremapped* turn FC for the turn source -- see `network_build_status.md`'s
2026-07-14 regression note under Step 3). The original three FCs are
therefore still untouched in `SDEADM.TRNLRS` in both Dev and QA; `06` hasn't
been needed yet in practice, but remains available for the eventual cleanup
once the `TRNLRS_network` builds are validated.
...
**Script:** `network_dataset/scripts/01_extract_network_config.py`
...
| Output | Purpose |
|---|---|
| `network_dataset/data/network_config.json` | Human-readable dump of all sources, attributes, evaluators, directions, and traffic config |
| `network_dataset/data/network_template.xml` | ArcGIS XML template — the authoritative input for recreating the dataset |
...
**Script:** `network_dataset/scripts/02_compare_schemas.py`
...
| Output | Purpose |
|---|---|
| `network_dataset/data/schema_comparison.json` | Full field-level diff: shared, only-in-old, only-in-new, type/length changes |
| `network_dataset/data/evaluator_field_map.json` | Per-evaluator status -- empty because all evaluators use VB Script expressions, not direct field evaluators |
...
> Select Case) is **correct about the live network but not about `network_dataset/data/network_template.xml`**,
...
**Status:** Complete -- `network_dataset/data/network_template.xml` has been updated

The following changes have been applied to `network_dataset/data/network_template.xml`:
...
**Follow-up:** `<CatalogPath>` has since been updated again, from
...
| `FEATURE_DATASET` | Target feature dataset for the new ND | `SDEADM.TRNLRS` at original build time (2026-06-26); now `SDEADM.TRNLRS_network` regardless of which environment `SDE_CONNECTION_UPDATE` points at, following the FD separation -- see note below |
...
**Feature dataset separation (2026-07-14):** the original 2026-06-26 build targeted
`SDEADM.TRNLRS` (the LRS feature dataset). `FEATURE_DATASET` in the script has since been
updated to `SDEADM.TRNLRS_network`, and `TRNLRS_street_network` has been (re)built there in
both Dev and QA -- see the note under [Overview](#overview) for the caveat that this
happened via script 03's fallback copy logic rather than a clean move of the existing
`SDEADM.TRNLRS` copies, which is why the turn source needed re-remapping in both
environments. Prod has not had this move applied yet.
...
references in `03_create_network_dataset.py` and `network_dataset/data/network_template.xml` accordingly.
...
See [`network_traffic_turns.md`](network_traffic_turns.md) for the original diagnosis and script.
(This link used to point at a `traffic_turns.md` that has never existed in this repository.)
...
1. Run `network_dataset/scripts/05_rebuild_traffic_turns.py`
...
(These notes were originally said to live in a `network_review.md`, which has never existed in
this repository. The surviving record is the summary below plus the QA review entries in
[`network_build_status.md`](network_build_status.md).)
...
**Prod caveat:** `network_dataset/scripts/04_sync_and_rebuild_network.py` and
`sync_network_edge_source()` now point at `SDEADM.TRNLRS_network`, but only Dev
and QA actually have `TRNLRS_TRN_STREET` etc. built there so far (see the note
...
**Corrected 2026-09-15. This note used to say the opposite, and it was wrong.** An earlier
version of this section stated that `TRNLRS_traffic_turn` does not need rebuilding on each LRS
refresh. **It does.** Turn features store their edge references as `Edge{N}FID`, which is the
**OBJECTID** of a feature in the registered edge source. `OBJECTID` is database-managed and is
not carried across by `Append`, so the `DeleteRows` + `Append` sync above reassigns every one
of them and every turn's edge reference becomes meaningless. The next `BuildNetwork` re-emits
`Cannot find edge element corresponding to turn identifier N` for the whole turn FC and
Properties shows `Turns: 0` -- silently, with a clean build message. This project has already
chased that exact regression three separate times.

`Edge{N}FCID` is safer but not free: it holds the edge FC's `DSID`, which survives
`DeleteRows`/`Append` (the FC object persists) but changes whenever the FC itself is recreated
(script 03's fallback copy, script 06's move, or any manual drop/recreate). **Re-run script 05
after anything that recreates `TRNLRS_TRN_STREET`, not just after edge geometry changes.**

So the real cadence after an LRS refresh is: sync the edge copy → re-remap turns (script 05) →
verify → swap → recreate the network dataset → Force Full Build → re-apply the SQL grants
(deleting the ND for the swap reassigns its `N_<id>`/`ND_<id>` registration IDs). See
[`qa_network_refresh_runbook.html`](qa_network_refresh_runbook.html) for the full procedure.

Which of three long-term fixes to adopt -- remap every cycle, make edge `OBJECTID`s stable via
a keyed update instead of truncate/reload, or store turns against a stable street key and
re-derive edge references at build time -- is an open decision that must be made **before prod
cutover**. See [`network_dataset_script_review.md` §D](network_dataset_script_review.md#d-turn-references-do-not-survive-an-lrs-refresh-structural).
...
**Corrected 2026-09-15.** This listing previously referenced `docs/network_review.md` and
`docs/traffic_turns.md`, neither of which has ever existed in this repository. Several scripts
and docs still contain inline references to those two names -- treat any such reference as
pointing at `network_traffic_turns.md` (for the turn work) or at the QA review notes recorded
inline in `network_build_status.md` (for the 2026-06-29 session). Below is what is actually here.

```
hrm_LRS/
├── scripts/
│   └── LRS_updates.py                            ← the LRS refresh pipeline
└── network_dataset/
    ├── data/
    │   ├── network_config.json                   ← generated by 01_extract_network_config.py
    │   ├── network_template.xml                  ← Python-evaluator template, corrected 2026-09-03
    │   ├── schema_comparison.json                ← generated by 02_compare_schemas.py
    │   └── evaluator_field_map.json              ← generated by 02_compare_schemas.py (empty, see script review §E)
    ├── docs/
    │   ├── network_dataset_migration_plan.md     ← this file: architecture and phase history
    │   ├── network_build_status.md               ← current build status and action items
    │   ├── network_dataset_script_review.md      ← line-by-line script review and open defects
    │   ├── network_dataset_sql_permissions.md    ← SQL grants and registration-ID procedure
    │   ├── network_traffic_turns.md              ← original turn-remap diagnosis
    │   ├── turn_rebuild_qa_test_runbook.md       ← QA execution procedure and evidence
    │   ├── street_network_meeting_overview.md    ← stakeholder-facing status summary
    │   ├── traffic_turn_staging_review_checklist.txt
    │   ├── roadmap_lrs_network.html              ← visual roadmap, milestones and open decisions
    │   ├── qa_network_refresh_runbook.html       ← how to refresh QA after an LRS correction
    │   ├── junction_network_workflow_esri_case.html  ← workflow + errors for Esri Case #04248942
