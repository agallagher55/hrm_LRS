...
class DynSegFeature:
    ...
    def update_safe_school_streets(
        ...
        pulls audit dates from E_SpeedLimit. The source is the
        safe-school-streets-specific segmented feature generated by
        :meth:`update_safe_school_streets_segmentation`.

        The segmentation overlays E_SafeSchoolStreets together with
        E_District and E_StreetClass, and those latter two are populated
        for the whole street network, not just tagged safe school streets.
        Left unfiltered, every currently-active street segment comes
        through this join — E_District/E_StreetClass rows with no matching
        E_SafeSchoolStreets record just get NULL audit fields instead of
        being dropped. PRESCREEN_CRIT is only ever populated on segments
        that came from E_SafeSchoolStreets, so filtering on it is what
        actually restricts the output to tagged safe school streets.
        """
        ...
        WHERE e.TO_DATE IS NULL
          AND e.PRESCREEN_CRIT IS NOT NULL
        """
