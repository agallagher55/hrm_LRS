...
class DynSegFeature:
    ...
    def update_safe_school_streets_segmentation(
        ...
        The main street dynamic segmentation is intentionally left unchanged.
        This overlay uses only E_SafeSchoolStreets to expose the safe school
        streets criteria fields (prescreening status, AAWT, transit route,
        through road access, active transportation connections) along the
        route network.
        """
    ...
    def update_safe_school_streets(
        ...
        ``out_feature`` should point to a feature class with the target fields
        ROUTEID, STR_NAME, FULL_NAME, PRESCREEN_CRIT, AAWT, TRANSIT_RTE,
        THROUGH_RD_ACCESS, ACTIVE_TRANS_INFRA_CONN, CRIT_STAT_COMMENT,
        LOCERROR, SDATE, SOURCE, SACC, ADDDATE, ADDBY, MODDATE, and MODBY.
        LOCERROR, SDATE, SOURCE, SACC, and the audit fields are not carried
        through by OverlayEvents, so they're joined back from
        E_SafeSchoolStreets (the defining event for this view) on ROUTEID,
        mirroring how the speed limit neighbourhood view pulls audit dates
        from E_SpeedLimit. The source is the safe-school-streets-specific
        segmented feature generated by
